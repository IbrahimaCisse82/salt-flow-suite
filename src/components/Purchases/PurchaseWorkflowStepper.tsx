import { FileText, ShieldCheck, CreditCard, Package, ArrowRight, CheckCircle } from "lucide-react";
import { cn } from "@/lib/utils";

interface PurchaseWorkflowStepperProps {
  pendingCount: number;
...
const steps = [
  { key: "pending", label: "Approbation", sublabel: "Valider les demandes", icon: ShieldCheck, countKey: "pendingCount" as const, managerOnly: true },
  { key: "approved", label: "Paiement", sublabel: "Payer le fournisseur", icon: CreditCard, countKey: "approvedCount" as const, managerOnly: false },
  { key: "to_receive", label: "Réception", sublabel: "Vérifier la livraison", icon: Package, countKey: "toReceiveCount" as const, managerOnly: false },
];

export const PurchaseWorkflowStepper = ({
  pendingCount,
  approvedCount,
  toReceiveCount,
  completedCount,
  ...
  onTabChange,
  isManager,
}: PurchaseWorkflowStepperProps) => {
  const counts = { pendingCount, approvedCount, toReceiveCount };

  const visibleSteps = steps.filter((s) => !s.managerOnly || isManager);
