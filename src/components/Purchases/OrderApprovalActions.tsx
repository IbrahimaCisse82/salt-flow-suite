...
export function OrderApprovalActions({ order }: Props) {
  ...
  const hasPayments = (order.total_paid ?? 0) > 0;
  const isReceived = ["received", "partially_received"].includes(order.status);
  ...
  // Annuler — bloqué si paiements ou réception
  if (["draft", "pending_approval", "rejected", "approved", "partially_paid"].includes(order.status)) {
    // Si des paiements existent, afficher un indicateur au lieu du bouton
    if (hasPayments) {
      return (
        <TooltipProvider>
          <Tooltip>
            ...
            <TooltipContent className="max-w-xs">
              <p>Cette commande ne peut pas être annulée car des paiements ont été effectués ({(order.total_paid ?? 0).toLocaleString()} F).</p>
              <p className="mt-1 text-xs">Pour annuler : enregistrez d'abord un retour de fonds via la comptabilité.</p>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
      );
    ...
    // Si la commande est reçue, bloquer aussi
    if (isReceived) {
      return null;
    }
