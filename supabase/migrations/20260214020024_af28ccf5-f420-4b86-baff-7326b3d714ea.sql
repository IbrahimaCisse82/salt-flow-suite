-- Fix the trigger to use 'tonnes' and handle actual salt_type values
...
BEGIN
  IF (TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND OLD.stock_updated = false)) AND NEW.deleted_at IS NULL THEN
    ...
    -- Handle both formats: 'gros'/'fin' and 'Sel gros'/'Sel fin'
    v_item_name := CASE 
...
-- Fix existing inventory items: change 'kg' to 'tonnes' and clean up names
UPDATE inventory_items 
SET unit_of_measure = 'tonnes' 
WHERE item_category = 'production' AND unit_of_measure = 'kg';

UPDATE inventory_items 
SET item_name = REPLACE(item_name, 'Sel - Sel ', 'Sel ')
WHERE item_category = 'production' AND item_name LIKE 'Sel - Sel %';